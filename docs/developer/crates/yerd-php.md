...
On **Windows** there is no FPM binary: `build_cmd` instead spawns `php-cgi.exe -b 127.0.0.1:<port>` (the FastCGI server) with `PHP_FCGI_MAX_REQUESTS=0` (php-cgi otherwise exits after N requests, which the supervisor would treat as a crash) and `PHP_INI_SCAN_DIR` pointing at a per-pool directory holding the supplemental ini (settings, directives, CA lines). `-c` is deliberately not passed, so the bundle's own `php.ini` (with `extension_dir`, the enabled extensions, and the install-time CA lines) stays active. The `-d extension=…` prefix is identical on both platforms; the containment of workers is via the Job Object from `yerd-supervise`, not a Unix process group.

Because one `php-cgi.exe` serves one request at a time, a PHP version is backed by `WORKERS_PER_VERSION` of them (`4` on Windows, `1` everywhere else, since FPM pre-forks its own workers and the Unix planner hands out one socket path per version). Each worker gets its own planned loopback port and its own supervised child; `PhpManager::ensure` advances a per-version round-robin cursor, spawns the worker it lands on **lazily**, and returns that worker's address, so `yerd-proxy` spreads requests across the workers with no proxy or IPC change. The count is a fixed compile-time constant with no `yerd.toml`, IPC or GUI surface. `snapshots()` aggregates the workers back to one row per version: `Running` with the first live worker's PID and address, or `Failed` with no PID once every stored worker has exited. A `Pool` is created only on a version's first successful worker spawn, which is what keeps "installed but never started" absent from the map and rendered as `Stopped` by the daemon.
