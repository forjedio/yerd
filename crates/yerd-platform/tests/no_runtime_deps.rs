...
//! `yerd-platform`'s own reachable set.
//!
//! Modelled on `crates/yerd-tls/tests/no_runtime_deps.rs`.
...
/// Best-effort current target triple. `rustc -vV` includes `host:`.
...
fn no_forbidden_crates_in_runtime_graph() {
    ...
    let yerd_platform_id = pkg_name
        .iter()
        .find(|(_, n)| **n == "yerd-platform")
        .map(|(id, _)| *id)
        .expect("yerd-platform must appear in cargo metadata");

    let mut reachable: HashSet<&str> = HashSet::new();
    let mut queue: VecDeque<&str> = VecDeque::new();
    queue.push_back(yerd_platform_id);
    reachable.insert(yerd_platform_id);
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "{forbidden} appeared in yerd-platform's runtime graph"
        );
    }
}
