...
//! `tokio` is allowed (the supervisor is intrinsically async).
//!
//! Modelled on `crates/yerd-platform/tests/no_runtime_deps.rs`.
...
const FORBIDDEN: &[&str] = &["anyhow", "reqwest", "openssl", "openssl-sys", "native-tls"];

const UNIQUE_VERSIONS: &[&str] = &["tokio", "time"];
...
fn no_forbidden_crates_in_runtime_graph_and_unique_versions() {
    ...
    let yerd_php_id = pkg_name
        .iter()
        .find(|(_, n)| **n == "yerd-php")
        .map(|(id, _)| *id)
        .expect("yerd-php must appear in cargo metadata");

    let mut reachable: HashSet<&str> = HashSet::new();
    let mut queue: VecDeque<&str> = VecDeque::new();
    queue.push_back(yerd_php_id);
    reachable.insert(yerd_php_id);
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "{forbidden} appeared in yerd-php's runtime graph"
        );
    }
