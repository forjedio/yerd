...
/// Spawns commands via `tokio::process::Command`, sets `kill_on_drop(true)`
/// so unexpected crashes of the daemon take FPM down with them.
...
impl ChildHandle for TokioChild {
    ...
    async fn kill(&mut self, signal: KillSignal) -> Result<(), io::Error> {
        #[cfg(unix)]
        {
            ...
            // `process_group(0)` was set at spawn time by the manager, so
            // the child's PID is also the process-group ID. Signal the
            // group so FPM workers are reaped along with the master.
            //
            // pid fits in i32 for any realistic value; reject pathological
            // PIDs explicitly.
            let pid_i32 =
        ...
        #[cfg(windows)]
        {
            // TODO(Phase 2): worker leak on Windows — needs job-object
            // teardown via yerd-helper.
            let _ = signal;
