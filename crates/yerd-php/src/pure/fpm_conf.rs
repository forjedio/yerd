...
/// Render the Windows php-cgi supplemental ini: the pool's validated settings,
/// per-version directives, and CA-bundle lines in plain `key = value` ini form.
///
/// php-cgi has no FPM pool config, so on Windows the manager loads this file via
/// `PHP_INI_SCAN_DIR` instead of rendering [`render_fpm_conf`]. The dump
/// extension and user extensions are passed on the command line (`-d`), not
/// here. Validation mirrors [`render_fpm_conf`] so the same allowlist gates both.
#[must_use]
pub fn render_win_ini(cfg: &PoolConfig) -> String {
    let mut out = String::with_capacity(256);
    let _ = writeln!(
        out,
        "; Generated by Yerd - supplemental php-cgi ini (PHP_INI_SCAN_DIR)."
    );
    for (key, value) in &cfg.ini {
...
mod tests {
    ...
    fn win_ini_renders_settings_directives_and_ca() {
        ...
        cfg.directives = vec![("xdebug.mode".to_owned(), "debug".to_owned())];
        cfg.ca_bundle = Some(PathBuf::from("/d/cacert.pem"));
        let ini = render_win_ini(&cfg);
        assert!(ini.contains("memory_limit = 512M"), "{ini}");
    ...
    #[test]
    fn win_ini_drops_unknown_settings() {
        let mut cfg = cfg_tcp();
        cfg.ini = vec![("not_a_real_setting".to_owned(), "x".to_owned())];
        let ini = render_win_ini(&cfg);
        assert!(!ini.contains("not_a_real_setting"), "{ini}");
    }
