...
fn no_forbidden_crates_in_runtime_graph_and_unique_versions() {
    ...
    let yerd_proxy_id = pkg_name
        .iter()
        .find(|(_, n)| **n == "yerd-proxy")
        .map(|(id, _)| *id)
        .expect("yerd-proxy must appear in cargo metadata");

    let mut reachable: HashSet<&str> = HashSet::new();
    let mut queue: VecDeque<&str> = VecDeque::new();
    queue.push_back(yerd_proxy_id);
    reachable.insert(yerd_proxy_id);
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "{forbidden} appeared in yerd-proxy's runtime graph"
        );
    }
